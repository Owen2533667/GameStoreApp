...
namespace GameStoreApp.Controllers
{
    ...
    public class GamePublisherController : Controller
    {
        ...
        private readonly IGamePublisherService _service;

        //constructor: A constructor that will accept a IGamePublisherService and set _service to the one passed as parameter.
        public GamePublisherController(IGamePublisherService service)
        {
            _service = service;
        }
        ...
        [AllowAnonymous]
        //Get: /Details/{Id}
        //Details: A asynchronous operation that returns a Task<IActionResult> called details. The method accepts two int values, the id of the publisher to get and pass to the view. The other int is an optional with a defualt of 0. It will get this vaule from the game details, when the user clicks on the <a> for game publisher. The second is used to store the return id, so when the user clicks the back button it will return to the game detials with that id.
        public async Task<IActionResult> Details(int id, int returnId = 0)
        {
            //Gets a model object from the related table using an id passed as parameter to the action result method.
            var dataDetails = await _service.GetPublisherByIdAsync(id);
        ...
        // A asynchronous method that accepts a GamePublisher model object. Also, bind the data from the form to model.
        [HttpPost]
        public async Task<IActionResult> Create([Bind("Logo, Name, Description")] GamePublisher publisher)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(int id, [Bind("Id, Logo, Name, Description")] GamePublisher publisher)
        {
