...
namespace GameStoreApp.Controllers
{
    ...
    public class GameDeveloperController : Controller
    {
        ...
        [AllowAnonymous]
        //Get: GamePublisher/Details/{Id}
        //Details: A asynchronous operation that returns a Task<IActionResult> called details. The method accepts two int values, the id of the developer to get and pass to the view. The other int is an optional with a defualt of 0. It will get this vaule from the game details, when the user clicks on the <a> tag for game developer. The second is used to store the return id, so when the user clcisk the back button it will return to the game detials with that id.
